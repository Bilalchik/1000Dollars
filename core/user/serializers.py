...
class OTPConfirmSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        otp_qs = OTP.objects.filter(code=code, user=user, if_used=False)

        if not otp_qs.exists():
            raise serializers.ValidationError("Неверный код или уже использован.")

        attrs['user'] = user
        attrs['otp_instance'] = otp_qs.first()
        return attrs

    def create(self, validated_data):
        otp = validated_data['otp_instance']
        otp.if_used = True
        otp.save()
        ...
        return {'detail': 'Код подтверждён успешно'}
